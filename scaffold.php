			...
			foreach($this->table AS $key => $value) {
				if (is_array($value)) {
					...
					if($column != $this->table['id_key'] ){
						$column_array[] = $column; 
						if($value['blob'] == 1){
							$text .= $this->html_chars("<p><b>" . $this->title($column) . ":</b><br /><textarea name='$column'><?php echo stripslashes(\$row['$column']) ?></textarea> \n");
						}
						else {
							$text .= "<p><b>" . $this->title($column) . ":</b><br /><input type='text' name='$column' value='<?php echo stripslashes(\$row['$column']) ?>' /> \n";
						}
					}
				}
			}
